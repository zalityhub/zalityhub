...
Filename:	lib/prince/princesendrsp.c
 ...
 * $Header: /home/hbray/cvsroot/fec/lib/prince/princesendrsp.c,v 1.3.4.2 2011/09/24 17:49:50 hbray Exp $
 *
 $Log: princesendrsp.c,v $
...
#ident "@(#) $Id: princesendrsp.c,v 1.3.4.2 2011/09/24 17:49:50 hbray Exp $ "
...
SendResponse(PiSession_t *sess)
{
    ...
    response->data[len] = 0;
    char *rsp = strchr(response->data, 0x1C);       // look for a trailing FS
    if ( rsp != NULL )
        len = (rsp - response->data);
    ...
    if ( len > 101 )
    {
        PiException(LogWarn, __FUNC__, eFailure, "Response length: %d too long; trimming to 101", len);
        len = 101;
    }

// right-space fill to 101 chrs
    rsp = alloca(102);
    memset(rsp, ' ', 102);
    rsp[101] = '\0';        // terminate string

    memcpy(rsp, response->data, len);
    len = 101;              // always send 101
