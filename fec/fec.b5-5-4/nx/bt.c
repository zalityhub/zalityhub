...
/* The BtOrder determines the maximum and minimum
 * number of entries (keys and pointers) in any
 * BtNode_t.  Every BtNode_t has at most BtOrder - 1 keys and
 * at least (roughly speaking) half that number.
 * Every leaf has as many pointers to data as keys,
 * and every internal BtNode_t has one more pointer
...
/* The BtQueue is used to print the tree in
 * level BtOrder, starting from the root
 ...
 * pointers, if the VerboseOutput flag is set.
 */
void
 ...
 * of the path from any BtNode_t to the root.
 */
int
 ...
 * line in level (rank) BtOrder, with the 
 * keys in each BtNode_t and the '|' symbol
 * to separate nodes.
 * With the VerboseOutput flag set.
 ...
 */
BtNode_t *
BtFindLeaf(BtNode_t * root, unsigned long key, boolean verbose)
{
	int i = 0;
	BtNode_t *c = root;
...
/* Finds and returns the BtRecord_t to which
 * a key refers.
 */
BtRecord_t *
BtFind(BtNode_t * root, unsigned long key, boolean verbose)
{
	int i = 0;
 ...
 * split a BtNode_t that is too big into two.
 */
int
BtCut(int length)
{
...
/* Creates a new BtRecord_t to hold the value
...
/* Creates a new general BtNode_t, which can be adapted
 * to serve as either a leaf or an internal BtNode_t.
 */
BtNode_t *
BtMakeNode(void)
{
...
/* Creates a new leaf by creating a BtNode_t
...
/* Helper function used in BtInsertIntoParent
 * to find the index of the parent's pointer to 
 * the BtNode_t to the left of the key to be inserted.
 */
int
...
/* Inserts a new pointer to a BtRecord_t and its corresponding
 * key into a leaf.
 ...
 * to a new BtRecord_t into a leaf so as to exceed
 * the tree's BtOrder, causing the leaf to be split
 * in half.
 */
BtNode_t *
...
/* Inserts a new key and pointer to a BtNode_t
 * into a BtNode_t into which these can fit
...
/* Inserts a new key and pointer to a BtNode_t
 * into a BtNode_t, causing the BtNode_t's size to exceed
 * the BtOrder, and causing the BtNode_t to split into two.
 */
BtNode_t *
BtInsertIntoNodeAfterSplitting(BtNode_t * root, BtNode_t * old_node, int left_index, unsigned long key, BtNode_t * right)
{
	 ...
	 * to hold everything in BtOrder, including
	 * the new key and pointer, inserted in their
	 * correct places. 
	 * Then create a new BtNode_t and copy half of the 
	 * keys and pointers to the old BtNode_t and
	...
	/* Create the new BtNode_t and copy
	 ...
	 * the old BtNode_t to the left and the new to the right.
	 */
...
/* Inserts a new BtNode_t (leaf or internal BtNode_t) into the B+ tree.
...
BtInsertIntoParent(BtNode_t * root, BtNode_t * left, unsigned long key, BtNode_t * right)
{
	...
	/* Case: leaf or BtNode_t. (Remainder of
	 * function body.)  
	 */
	...
	/* Harder case:  split a BtNode_t in BtOrder 
...
BtInsert(BtNode_t * root, unsigned long key, unsigned long value)
{
	...
	/* Create a new BtRecord_t for the
	 * value.
	 */
 ...
 * the index of a BtNode_t's nearest neighbor (sibling)
 * to the left if one exists.  If not (the BtNode_t
...
/* Coalesces a BtNode_t that has become
 * too small after deletion
 * with a neighboring BtNode_t that
...
BtCoalesceNodes(BtNode_t * root, BtNode_t * n, BtNode_t * neighbor, int neighbor_index, int k_prime)
{
	...
	/* Swap neighbor with BtNode_t if BtNode_t is on the
	...
	/* Case:  nonleaf BtNode_t.
	 * Append k_prime and the following pointer.
	 * If there is room in the neighbor, append
	 * all pointers and keys from the neighbor.
	 * Otherwise, append only cut(BtOrder) - 2 keys and
	 * cut(BtOrder) - 1 pointers.
	 */
 ...
 * small BtNode_t's entries without exceeding the
 * maximum
 */
BtNode_t *
 ...
 * Removes the BtRecord_t and its key and pointer
...
BtDeleteEntry(BtNode_t * root, BtNode_t * n, unsigned long key, void *pointer)
{
	...
	/* Case:  deletion from a BtNode_t below the root.
	 * (Rest of function body.)
	 */

	/* Determine minimum allowable size of BtNode_t,
	 * to be preserved after deletion.
	 */

	min_keys = n->is_leaf ? BtCut(BtOrder - 1) : BtCut(BtOrder) - 1;

	/* Case:  BtNode_t stays at or above minimum.
	 * (The simple case.)
	 */
	...
	/* Case:  BtNode_t falls below minimum.
	 * Either coalescence or redistribution
	 * is needed.
	 */

	/* Find the appropriate neighbor BtNode_t with which
	 * to coalesce.
	 * Also find the key (k_prime) in the parent
	 * between the pointer to BtNode_t n and the pointer
	 * to the neighbor.
	 */
...
int
BtNodeWalk(BtNode_t *node, BtNodeWalker_t walker, void *arg)
{
	int ret = 0;
	...
	if ( node->is_leaf )
	{
		...
		{
			if ( (ret = (*walker)(node->keys[i], node->pointers[i], arg)) < 0 )
				break;		// abort
		}
	}
	else
	{
		for(int i = 0; i <= node->num_keys; ++i)
		{
			if ( (ret = BtNodeWalk(node->pointers[i], walker, arg)) < 0 )
				break;		// abort
		}
	}
...
BtMain(BtNode_t *root)
{
	...
	while (scanf("%c", &instruction) != EOF)
	{
		...
		case 't':
			// while (getchar() != (int)'\n');
			BtPrintTree(root, stdout);
			//BtNodeWalk(root, NodePrinter, NULL);
			break;
		case 'v':
			// while (getchar() != (int)'\n');
