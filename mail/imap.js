...
// Implementation
      ...
      self.imap.once('error', (err) => {
        ...
        self._error(`Failed to connect err=${self.stringify(err)}`);
        reject(err, options);
        self.imap.end();
        delete self.imap;
      });
  ...
  open(options) {
    ...
    self._trace(`Open: ${self.stringify(options, null, 4)}`);
...
// Implementation
      ...
      self.imap.openBox(options.box, options.readonly, (err, box) => {
        if (err) {
          self._error(`Failed to openBox err=${self.stringify(err)}`);
          reject(err);
        }
  ...
  search(options) {
    ...
    self._trace(`Search: ${self.stringify(options, null, 4)}`);

    self._trace(`Search promising`);
    return new Promise(function (resolve, reject) {
      self.imap.search(options.search, (err, messages) => {
        if (err) {
          self._error(`Search ${options.search} failed with err=${self.stringify(err)}`);
          reject(err);
        }
  ...
  fetch(options) {
    ...
    self._trace(`Fetch: ${self.stringify(options, null, 4)}`);
...
// Implementation
      ...
      self._fetch.once('error', (err) => {
        self._error(`Fetch err=${self.stringify(err)}`);
        reject(err);
      });
  ...
  parse(email) {
    ...
    function getBidValue(header) {
      try {
        ...
        if (self.nx.isArray(bid))
          bid = bid.join(' ');
        let idx = bid.toLowerCase().indexOf('boundary=');
        if (idx > 0)
          bidValue = self.nx.replaceAll(bid.slice(idx + 10), ['=', '"'], ['', '']);
      } catch (err) {
        errors.push(`${self.stringify(header)}\n${err.toString()}`);
      }
    }
    ...
    function collectHeader(lines, i) {
      for (i = i + 1; i < lines.length; ++i) {
        ...
        if (!self.nx.isArray(header.value))
    ...
    let header, line, i, j;
    let body = self.nx.CloneObj(email.body);
    ...
    let hdrs = self.nx.CloneObj(headers);
    let unks = self.nx.CloneObj(unknowns);
    headers = [];
    unknowns = [];
    let bid_line;
    for (i = i + 1; i < lines.length; ++i) {
