...
exports.Corpus = class Corpus {
  ...
  constructor(names, texts, useDefaultStopwords = true, customStopwords = [], K1 = 2.0, b = 0.75) {
    this._stopwords = new Stopwords.Stopwords(useDefaultStopwords, customStopwords);
    this._K1 = K1;
    this._b = b;
    this._documents = new Map();
    for (let i = 0; i < texts.length; i++) {
      this._documents.set(names[i], new Document.Document(texts[i]));
    }
  ...
  getResultsForQuery(query) {
    ...
    const terms = new Document.Document(query).getUniqueTerms();
