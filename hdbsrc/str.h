...
StringNewCpy(char *str)
{
  String_t       *tmp = StringNew(strlen(str) + 10);
...
#define StringArrayStatic(name, nmemb, \
     size) static StringArray_t *name = NULL; if ( name == NULL ) name = StringArrayNew(nmemb, size)
