...
------------------------------------------------------------------------------
 * $Author: jtfulle $
 * $Date: 2018/02/05 16:33:53 $
 * $Header: /home/gssvc/cvsroot/gbs/lib/nx/include/json.h,v 1.4 2018/02/05 16:33:53 jtfulle Exp $
 *
 $Log: json.h,v $
 Revision 1.4  2018/02/05 16:33:53  jtfulle
 Rework of va_start and va_ends

 Revision 1.3  2015/11/04 19:01:01  hbray
 Merge scalable TIP

 Revision 1.2  2013/11/19 15:13:25  gssvc
 Merged gbs_xscale/lib/nx into CVS Attic(HEAD)

 Revision 1.1.2.1  2013/10/28 13:18:51  hbray
 New

 Revision 1.1.2.1  2013/10/28 12:41:47  hbray
 New

 Revision 1.1.1.1  2013/05/23 00:58:47  hbray
 initial import

 Revision 1.1.1.1  2013/02/06 21:57:36  hbray
 initial import
...
#ident "@(#) $Id: json.h,v 1.4 2018/02/05 16:33:53 jtfulle Exp $ "
...
#define _JsonNew(node, name, ...) ObjectNew(Json, node, name, ##__VA_ARGS__)
#define JsonVerify(var) ObjectVerify(Json, var)
#define JsonDelete(var) ObjectDelete(Json, var)
extern ObjectNode_t *JsonNodeList;
...
// For analysing failed parses. This returns a pointer to the parse error. You'll probably need to look a few chars back to
// make sense of it. Defined when JsonParse() returns 0. 0 when JsonParse() succeeds.
...
extern Json_t  *JsonClone(Json_t * json);
...
#define JsonAddHexNumber(object,name,n) JsonAddString(object, name, "%08X", (n))


static inline char *
JsonDeCookString(Json_t * object)
{
 if (object == NULL || object->node->type != cJSON_String)
  return NULL;
 return DecodeUrlCharacters(object->node->valuestring, NULL);
};

static inline void
JsonAddStringV(Json_t * object, char *name, char *fmt, va_list ap)
{
 JsonAddItem(object, name, JsonCreateStringV(fmt, ap));
}

static inline void
JsonAddString(Json_t * object, char *name, char *fmt, ...)
{
 va_list         ap;
...
static inline void
JsonAddCookedString(Json_t * object, char *name, char *raw, int rawlen)
{
 JsonAddString(object, name, "%s", EncodeUrlCharacters(raw, rawlen));
}

static inline void
JsonAddPointer(Json_t * object, char *name, void *ptr)
{
 JsonAddString(object, name, "%p", ptr);
}


extern Json_t  *JsonPushObject(Json_t * node, char *name, ...);
